package day17arraylists;

import java.util.ArrayList;
import java.util.List;

public class ArrayLists03 {

    public static void main(String[] args) {
        List<String> cities = new ArrayList<>();
        cities.add("Miami");
        cities.add("Istanbul");
        cities.add("Kayseri");
        cities.add("Almaty");
        cities.add("Kayseri");
        System.out.println(cities);

        //remove() methodu index ile kullanilirsa o index'teki elemani siler
        String n = cities.remove(1);
        System.out.println(n);// Istanbul
        System.out.println(cities);

        //remove(o:) methodu eleman ile kullanilirsa ilk gorunumu siler
        //remove() methodu eleman ile kullanilirsa size o elemani silip silmedigini ifade eden
        //true veya false verir.
        //Eger eleman List'te var ise o elemani siler ve suze true der
        //Eger eleman List'te yok ise o elemani silemediginden size false verir
        //Clear ile arasindaki fark clear de herseyi silersiniz remove da tek tek sileriz.

        boolean p = cities.remove("Kayseriye");
        System.out.println(p);
        System.out.println(cities);
    }
}
